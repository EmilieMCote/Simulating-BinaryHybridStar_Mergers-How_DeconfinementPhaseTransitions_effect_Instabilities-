...
RHODATA   = SD.gf.xy['Temp']
Temp_max = SD.ts.maximum['Temp'].y[0]
...
#for i in range(0,1,1):
    ...
    RHOVAR=(RHODATA.get_time(times[i]).to_UniformGridData([npoints, npoints], x0=[xmin, ymin], x1=[xmax, ymax], resample=False).data)
    
    #set the fig and axes size
    fig, axs= plt.subplots(nrows=1, ncols=1,figsize=(10,10))

    #plot the density
    rhoplot=axs.imshow(RHOVAR/Temp_max.T, extent=extent, cmap=cmap_Temp, interpolation=int_mthd, vmin=-5, vmax=0)
...
with imageio.get_writer('%s/Temp.gif'%datadir, mode='I') as writer:
